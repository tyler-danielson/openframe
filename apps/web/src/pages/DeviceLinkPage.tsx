  ...
  const requestCode = useCallback(async () => {
    ...
    try {
      ...
      pollTimerRef.current = setInterval(async () => {
        try {
          const pollRes = await fetch(`${API_BASE}/auth/device-code/poll`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
          ...
          if (status === "approved" && kioskToken) {
            ...
            // Redirect to kiosk
            setTimeout(() => {
              window.location.href = `/kiosk/${kioskToken}`;
            }, 1500);
          ...
          // Silent failure — retry next poll
        }
      }, 4000);
    } catch (err) {
      setState("error");
          ...
          <>
            ...
            {qrDataUrl && (
              <div style={{
                ...
                <img src={qrDataUrl} alt="QR Code" style={{ display: "block", width: 248, height: 248 }} />
              </div>
            )}
