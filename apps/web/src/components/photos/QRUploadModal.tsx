...
export function QRUploadModal({ isOpen, onClose, albumId, albumName }: QRUploadModalProps) {
  ...
  const generateToken = async () => {
    ...
    setError(null);
    try {
      const result = await api.createUploadToken(albumId);
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="text-center space-y-2 mb-6">
                ...
                <p className="text-sm text-muted-foreground">
                  Opens a page to upload photos directly to this album
                </p>
              </div>
              ...
              <div className="mt-6 w-full space-y-2">
                ...
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  <CheckCircle className="h-4 w-4 text-green-500" />
                  <span>Works on same network only</span>
                </div>
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  <CheckCircle className="h-4 w-4 text-green-500" />
                  <span>Link expires when you close this</span>
                </div>
              </div>
            </>
          ) : (
