 ...
 * Refactored from the original agenda-generator.ts to use the new template engine interface.
 */
...
export class DailyAgendaGenerator implements TemplateGenerator {
  async generate(options: TemplateOptions): Promise<TemplateResult> {
    ...
    const showLocation = config.showLocation ?? true;
    const showDescription = config.showDescription ?? false;
    const notesLines = config.notesLines ?? 20;
    ...
    return {
      buffer,
      filename: `Agenda ${format(date, "yyyy-MM-dd")}`,
      pageCount: 1,
    };
  }
  ...
  validateConfig(config: RemarkableTemplateConfig): { valid: boolean; errors: string[] } {
    ...
    if (config.notesLines !== undefined) {
      if (config.notesLines < 0 || config.notesLines > 50) {
        errors.push("notesLines must be between 0 and 50");
      }
    }
